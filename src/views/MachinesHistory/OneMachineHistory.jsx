import { useParams } from "react-router-dom"
import {
  deleteMachineStateHistory,
  getOneMachineStateHistory,
  updateMachineStateHistory,
} from "../../redux/actions/machinesStatesHistory"
import { useDispatch, useSelector } from "react-redux"
import { Fragment, useEffect, useState, useRef } from "react"
import Title from "../../components/Utils/Title.jsx"
import {
  BreadCrumb,
  Card,
  Button,
  Toast,
  SplitButton,
  ProgressSpinner,
} from "primereact"
import { inputText } from "../../components/Utils/Form/Form"
import { ToastUpdateSuccess } from "../../components/Utils/Toast.jsx"
import { useNavigate } from "react-router"
import DialogDelete from "../../components/Utils/DialogDelete"
import useCopyToClipboard from "../../hooks/useCopyToClipboard"

const OneMachineHistory = () => {
  const { machineStateHistoryId } = useParams()
  const dispatch = useDispatch()
  const navigate = useNavigate()
  const oneMachineStateHistory = useSelector(
    (state) => state.getOneMachineStateHistory.data.machineStateHistory
  )
  const isLoading = useSelector(
    (state) => state.getOneMachineStateHistory.loading
  )
  const { copyToClipboard, toastCopy } = useCopyToClipboard()

  const [data, setData] = useState({})

  const toastSuccess = useRef(null)
  const toastError = useRef(null)
  const [dialogDeleteVisible, setDialogDeleteVisible] = useState(false)

  const breadCrumbItems = [
    {
      label: oneMachineStateHistory?.Machine?.type,
      url: `/one-machine/${oneMachineStateHistory?.Machine?.machineId}`,
    },
    {
      label: oneMachineStateHistory?.machineStateHistoryId,
      url: `/one-machine-history/${oneMachineStateHistory?.machineStateHistoryId}`,
    },
  ]
  const breadCrumbHome = { icon: "pi pi-home", url: "/" }

  const handleUpdate = async (e) => {
    e.preventDefault()
    try {
      await dispatch(updateMachineStateHistory(machineStateHistoryId, data))
      ToastUpdateSuccess(
        toastSuccess,
        "Machine History modifiée avec succès",
        false
      )
    } catch (error) {
      console.log(error)
    }
  }

  const handleDialogDelete = () => {
    setDialogDeleteVisible(true)
  }

  const handleDelete = async () => {
    try {
      await dispatch(deleteMachineStateHistory(machineStateHistoryId))
      ToastUpdateSuccess(
        toastSuccess,
        "Machine History supprimée avec succès",
        false
      )
      setTimeout(() => {
        navigate(`/one-machine/${oneMachineStateHistory?.Machine?.machineId}`)
      }, 500)
    } catch (error) {
      console.log(error)
    }
  }

  const actionItems = [
    {
      label: "Supprimer",
      icon: "pi pi-trash text-red-600",
      command: () => {
        handleDialogDelete()
      },
    },
  ]

  useEffect(() => {
    dispatch(getOneMachineStateHistory(machineStateHistoryId))
  }, [dispatch, machineStateHistoryId])

  return (
    <Fragment>
      <Title title={machineStateHistoryId?.machineStateHistoryId} />
      <div className="flex justify-content-between my-2">
        <BreadCrumb model={breadCrumbItems} home={breadCrumbHome} />
      </div>
      <Card className="card w-8 m-auto">
        <div className="flex justify-content-between">
          <span className="p-input-icon-left flex align-items-center">
            <h3>{machineStateHistoryId?.machineStateHistoryId}</h3>
          </span>
          <span className="p-input-icon-left">
            <SplitButton
              label="Action"
              className="p-button-outlined p-button-secondary"
              model={actionItems}
            />
          </span>
        </div>
        <Fragment>
          {isLoading ? (
            <ProgressSpinner />
          ) : (
            <Card>
              <form
                onSubmit={handleUpdate}
                className="p-fluid flex w-full justify-content-around"
              >
                <div className="flex w-full flex-column">
                  <div className="flex w-full flex-column align-items-center md:flex-row md:justify-content-center md:align-items-start">
                    <div className="w-6">
                      <div className="field">
                        <h5 className="mb-1">{"ID"}</h5>
                        <div className="p-inputgroup">
                          {inputText(
                            data,
                            setData,
                            oneMachineStateHistory?.machineStateHistoryId,
                            "machineStateHistoryId",
                            "ID",
                            true
                          )}
                          <Button
                            icon="pi pi-copy"
                            type="button"
                            className="p-button-secondary"
                            onClick={() =>
                              copyToClipboard(
                                oneMachineStateHistory?.machineStateHistoryId
                              )
                            }
                          />
                        </div>
                      </div>
                      <div className="field">
                        <h5 className="mb-1">{"Type"}</h5>
                        <div className="p-inputgroup">
                          {inputText(
                            data,
                            setData,
                            oneMachineStateHistory?.Machine?.type,
                            "type",
                            "Type",
                            true
                          )}
                          <Button
                            icon="pi pi-copy"
                            type="button"
                            className="p-button-secondary"
                            onClick={() =>
                              copyToClipboard(
                                oneMachineStateHistory?.Machine?.type
                              )
                            }
                          />
                        </div>
                      </div>
                      <div className="field">
                        <h5 className="mb-1">{"Code"}</h5>
                        <div className="p-inputgroup">
                          {inputText(
                            data,
                            setData,
                            oneMachineStateHistory?.Error?.errorCode,
                            "errorCode",
                            "Code erreur",
                            true
                          )}
                          <Button
                            icon="pi pi-copy"
                            type="button"
                            className="p-button-secondary"
                            onClick={() =>
                              copyToClipboard(
                                oneMachineStateHistory?.Error?.errorCode
                              )
                            }
                          />
                        </div>
                      </div>
                      <div className="field">
                        <h5 className="mb-1">{"Etat"}</h5>
                        <div className="p-inputgroup">
                          {inputText(
                            data,
                            setData,
                            oneMachineStateHistory?.state,
                            "state",
                            "Etat",
                            true
                          )}
                          <Button
                            icon="pi pi-copy"
                            type="button"
                            className="p-button-secondary"
                            onClick={() =>
                              copyToClipboard(oneMachineStateHistory?.state)
                            }
                          />
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="flex justify-content-center mt-4">
                    <Button
                      label={"Mettre à jour"}
                      className="p-button-sm p-component p-button-raised p-button-success w-auto"
                    />
                  </div>
                </div>
              </form>
            </Card>
          )}
          <Toast ref={toastSuccess} position="top-center" />
          <Toast ref={toastError} position="top-center" />
          <Toast ref={toastCopy} position="top-center" />
        </Fragment>
      </Card>
      <DialogDelete
        dialogDeleteVisible={dialogDeleteVisible}
        setDialogDeleteVisible={setDialogDeleteVisible}
        handleDelete={handleDelete}
        title={"Suppression historique machine"}
        description={
          "La suppression entrainera la perte définitive de cet historique et de toutes données le concernant. Etes vous sur de vouloir continuer ?"
        }
      />
    </Fragment>
  )
}

export default OneMachineHistory
